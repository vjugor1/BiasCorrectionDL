...
class RRDB(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, model_channels, model_upscaling_channels, model_blocks, scale) -> None:
        """
        ...
        # self.scheduler_step = optimizer_scheduler_step
        self.channels = model_channels
        # self.ssim = StructuralSimilarityIndexMeasure(data_range=8.0)

        # Define model parameters
        upscaling_factor = scale
        upscaling_channels = model_upscaling_channels
        blocks = model_blocks

        # Define the neural network model using nn.Sequential
        self.model = nn.Sequential(
            nn.ReplicationPad2d(1),
            nn.Conv2d(in_channels, upscaling_factor * upscaling_factor * upscaling_channels, kernel_size=3),
            ...
            nn.ReplicationPad2d(1),
            nn.Conv2d(self.channels, out_channels=out_channels, kernel_size=3),
        )
...
class VGG19FeatureExtractor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.stack([self.vgg(x[..., c, :, :].unsqueeze(1).repeat(1, 3, 1, 1)) for c in range(x.shape[-3])], dim=-3)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, out_channels: int, feature_maps: int = 64) -> None:
        """
        ...
        self.conv_blocks = nn.Sequential(
            self._make_double_conv_block(out_channels, feature_maps, first_batch_norm=False),
...
class ESRGAN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, scale, model_channels=64, model_upscaling_channels=3, model_blocks=16, disc_feature_map=64) -> None:
        super().__init__()
        # self.lr = optimizer_lr
        # self.scheduler_step = optimizer_scheduler_step

        # Load pretrained generator
        self.generator = RRDB(in_channels=in_channels, out_channels=out_channels,
            model_channels=model_channels, model_upscaling_channels=model_upscaling_channels, model_blocks=model_blocks, scale=scale
        )

        self.discriminator = Discriminator(out_channels, disc_feature_map)
