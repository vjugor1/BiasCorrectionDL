...
def main(cfg: DictConfig):
    ...
    # Evaluate saved model checkpoint
        ...
        # Продолжить обучение
        trainer.fit(model_module, datamodule=dm, ckpt_path=cfg.training.checkpoint)
        trainer.test(model_module, datamodule=dm, ckpt_path="best")
...
def setup_model(dm, config):
    model = load_downscaling_module(
        ...
        val_target_transform=[dm.denorm_mask, dm.denorm_mask, dm.denorm_mask, dm.mask, dm.denorm_mask, dm.denorm_mask, dm.mask],
        test_target_transform=[dm.denorm_mask, dm.denorm_mask, dm.denorm_mask, dm.denorm_mask, dm.denorm_mask, dm.denorm_mask, dm.denorm_mask,dm.denorm_mask,dm.mask]
    )
    return model
