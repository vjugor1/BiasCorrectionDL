...
def main(cfg: DictConfig):
    ...
    if cfg.training.checkpoint is not None:
        if cfg.model.architecture == 'diffusion':
            ...
            normalized_clim = prepare_ynet_climatology(
                dm, path_to_elevation, out_vars)

            model = YnetLitModule.load_from_checkpoint(
                cfg.training.checkpoint,
                net=model.net,
                optimizer=model.optimizer,
                lr_scheduler=None,
            ...
            model = GANLitModule.load_from_checkpoint(
                cfg.training.checkpoint,
                net=model.net,
                optimizer=model.optimizer,
                lr_scheduler=None,
            ...
            model = ESRGANLitModule.load_from_checkpoint(
                cfg.training.checkpoint,
                net=model.net,
                optimizer=model.optimizer,
                lr_scheduler=None,
        ...
        return model, dm#trainer.predict(model, datamodule=dm, dataloaders=dm.predict_dataloader())#
...
def setup_model(dm, config):
    model = load_downscaling_module(
        ...
        train_loss=tuple(config.training.train_loss) if len(
            config.training.train_loss) > 1 else str(config.training.train_loss[0]),
...
def setup_trainer(config, default_root_dir):
    ...
    #             f"{default_root_dir}/logs", f"version_{logger.version}", "checkpoints"),
    #         monitor=early_stopping,
